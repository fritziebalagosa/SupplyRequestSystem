...
$stmt = $conn->prepare("SELECT r.*, u.first_name, u.last_name FROM requests r JOIN users u ON r.requester_id = u.id WHERE r.id = ? AND r.college_office_id = ? LIMIT 1");
$stmt->bind_param("ii", $id, $college_office_id);
$stmt->execute();
...
<body>
    ...
    <div class="container-main">
        ...
        <?php
        // Only show action form to dean when request is in actionable statuses
        $can_act = false;
        $role = $_SESSION['role'] ?? '';
        if ($role === 'dean' && in_array($request['status'], ['pending_dean', 'pending_head', 'pending_officer'])) {
            $can_act = true;
        }
        ?>
        <?php if ($can_act): ?>
        <div class="info-card">
            ...
            <form method="POST" action="dean_requests.php">
                <input type="hidden" name="request_id" value="<?= $request['id'] ?>">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($csrf_token) ?>">
                <div class="mb-3">
                    <label class="form-label-minimal">Comment (optional)</label>
                    <textarea name="comment" class="form-control-minimal" rows="3"></textarea>
                </div>
                <div class="form-actions">
                    <button type="submit" name="action" value="approve" class="btn-minimal btn-success-minimal"><i class="bi bi-check-circle"></i> Approve</button>
                    <button type="submit" name="action" value="return" class="btn-minimal btn-secondary-minimal"><i class="bi bi-arrow-return-left"></i> Return with Comment</button>
